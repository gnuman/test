...
define("tinymce/tableplugin/Dialogs", [
	...
	return function(editor) {
		...
		function getTableStyle(classes) {
			var tableStyle = '';
			each(classes, function(class_name) {
				if (class_name !== 'table-floatleft' && class_name !== 'table-floatright' && class_name !== 'table-center' && class_name.slice(0, 4) !== 'mce-' && class_name.slice(0, 14) !== 'table-padding-') {
					tableStyle = class_name;
				}
			});
		...
		function getAlignment(classes) {
			var alignment = '';
			each(classes, function(class_name) {
				if (class_name === 'table-floatleft') {
					alignment = 'left';
				} else if (class_name === 'table-floatright') {
					alignment = 'right';
				} else if (class_name === 'table-center') {
					alignment = 'center';
				}
			});
		...
		function getPadding(classes) {
			var padding = '';
			each(classes, function(class_name) {
				if (class_name === 'table-padding-compact') {
					padding = 'table-padding-compact';
				} else if (class_name === 'table-padding-cozy') {
					padding = 'table-padding-cozy';
				} else if (class_name === 'table-padding-comfortable') {
					padding = 'table-padding-comfortable';
				} else if (class_name === 'table-padding-roomy') {
					padding = 'table-padding-roomy';
				}
			});
		...
		self.table = function(isProps) {
			...
			function onSubmitTableForm() {
				...
				if (data.align === 'left') {
					classes += ' table-floatleft';
				} else if (data.align === 'right') {
					classes += ' table-floatright';
				} else if (data.align === 'center') {
					classes += ' table-center';
				}

				if (data.padding !== '') {
					classes += ' ' + data.padding;
				}
			...
			if (isProps === true) {
				...
				if (tableElm) {
					data = {
						...
						'class': getTableStyle(dom.getAttrib(tableElm, 'class').split(' ')),
						align: getAlignment(dom.getAttrib(tableElm, 'class').split(' ')),
						padding: getPadding(dom.getAttrib(tableElm, 'class').split(' ')),
					};
				}
			} else {
			...
			generalTableForm = {
				...
				items: [
...

					{
						...
						values: [
							{text: 'None', value: ''},
							{text: 'Compact', value: 'table-padding-compact'},
							{text: 'Cozy', value: 'table-padding-cozy'},
							{text: 'Comfortable', value: 'table-padding-comfortable'},
							{text: 'Roomy', value: 'table-padding-roomy'}
						]
					}
				]
			};
